...
const ProjectListView: React.FunctionComponent<ProjectListViewProps> = ({ projects }: ProjectListViewProps ):React.ReactElement => {
    ...
    const filterProjects = (projects:Project[]):Project[] => {
        ...
        projects.forEach((project) => {
            ...
            const managerMatches = manager === '' ? true : (project.manager && project.manager.toLowerCase() === manager.toLowerCase())
...
const ProjectFilter: React.FunctionComponent<ProjectFilterProps> = ({ title, handleTitleChange, startDate, handleStartDateChange, endDate, handleEndDateChange, manager, handleManagerChange }: ProjectFilterProps):React.ReactElement => {
    return (
        <div className="container shadow bg-gray-200 mt-3">
                <div className="flex md:flex-row flex-col my-2">
                    <div className="flex-auto text-left md:text-center">
                        <label className="text-2xl text-gray-800" htmlFor="title">Title:</label>
                        <input className="mx-1 border rounded-md focus:ring-2 outline-none bg-gray-300 text-gray-700" type="text" id="title" value={title} onChange={handleTitleChange}/>
                    </div>
                    <div className="flex-auto text-left md:text-center">
                        <label className="text-2xl text-gray-800" htmlFor="start-date">Start date:</label>
                        <input className="mx-1 border rounded-md focus:ring-2 outline-none bg-gray-300 text-gray-700" type="date" id="start-date" value={startDate} onChange={handleStartDateChange}/>
                    </div>
                    <div className="flex-auto text-left md:text-center">
                        <label className="text-2xl text-gray-800" htmlFor="end-date">End date:</label>
                        <input className="mx-1 border rounded-md focus:ring-2 outline-none bg-gray-300 text-gray-700" type="date" id="end-date" value={endDate} onChange={handleEndDateChange}/>
                    </div>
                    <div className="flex-auto text-left md:text-center">
                        <label className="text-2xl text-gray-800" htmlFor="manager">Manager:</label>
                        <select name="manager" id="manager" className="mx-1 border rounded-md focus:ring-2 outline-none bg-gray-300 text-gray-700" value={manager} onChange={handleManagerChange}>
                            <option value=""></option>
...
const ProjectTable: React.FunctionComponent<ProjectListViewProps> = ({ projects }: ProjectListViewProps): React.ReactElement => {
    ...
    return (
        <table className="shadow-lg mt-1 w-full text-left">
            <thead>
                <tr>
                    {headers.map((header: string) => createHeader(header))}
                </tr>
            </thead>
            <tbody>
